...
def build_canvas_doc(man, cur_can):
    ...
    doc = OrderedDict()
    print("processing manifest and canvas cutout dictionary")
...
def build_curation_doc(cur, activity, canvas_doc=None, cur_can_idx=None):
    ...
    # PMB
    print("Curation URL",cur['@id'])
    if ('curationLabel' in cur):
        doc['curationLabel'] = cur['label']
    else:
        print("curation label is missing, maybe this is OK")
        doc['curationLabel'] = "none"
    if canvas_doc:
        print("Looking at canvas doc")
        doc['curationThumbnail'] = canvas_doc['canvasThumbnail']
    else:
        print("It's not a canvas doc")
